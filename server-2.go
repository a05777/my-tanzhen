...
func main() {
	...
	r.GET("/api", handleAPIStatus) // 新增的 API 接口
...
func handleAPIStatus(c *gin.Context) {
    var results []struct {
        ...
        Swap    float64 `json:"swap"`
        Latency int64   `json:"latency"`
        LastUp  time.Time `json:"last_update"`
    }

    // 复用你 WS 里的那套 SQL 逻辑
    query := `SELECT c.id, c.name, c.line_info as line, c.price, c.expiry_date as expiry, 
              m.cpu, m.ram, m.disk, m.swap, m.latency, c.last_update FROM client_infos c 
              INNER JOIN metrics m ON m.client_id = c.id 
              WHERE m.id = (SELECT MAX(id) FROM metrics WHERE client_id = c.id) 
              AND c.last_update > ?`
    
    // 查询最近 5 分钟内有更新的节点（宽容度高一点）
    db.Raw(query, time.Now().Add(-5*time.Minute)).Scan(&results)
